	...
	implements AMI_StandardHandler
{
	...
	public void inviter_client_excep(org.omg.Messaging.ExceptionHolder excep_holder)
	{
		...
		if(! this._is_local())
		{
			...
			try
			{
				org.omg.CORBA.portable.OutputStream _os = _request( "inviter_client_excep", true);
				((org.omg.CORBA_2_3.portable.OutputStream)_os).write_value (excep_holder );
				_is = _invoke(_os);
				return;
			}
		...
		{
			org.omg.CORBA.portable.ServantObject _so = _servant_preinvoke( "inviter_client_excep", _opsClass );
			if( _so == null )
				throw new org.omg.CORBA.UNKNOWN("local invocations not supported!");
			AMI_StandardHandlerOperations _localServant = (AMI_StandardHandlerOperations)_so.servant;
			try
			{
			_localServant.inviter_client_excep(excep_holder);
			}
			finally
			{
				_servant_postinvoke(_so);
	...
	public void inviter_client(boolean ami_return_val)
	{
		...
		else
		{
			...
			try
			{
			_localServant.inviter_client(ami_return_val);
			}
			finally
			{
				_servant_postinvoke(_so);
